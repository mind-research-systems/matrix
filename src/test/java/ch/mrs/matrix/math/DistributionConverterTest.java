...
public class DistributionConverterTest {
	...
	private void assertBinaryDistribution(List<BinaryRange<Integer>> result) {
		...
		assertEquals(0.2,result.get(0).getTriggerValue(),0.0);
		assertEquals(0.4,result.get(1).getTriggerValue(),0.0);
		assertEquals(0.4,result.get(2).getTriggerValue(),0.0);
		assertEquals(0.8,result.get(3).getTriggerValue(),0.0);
	}
